...
export const ShowSceneEffect: Effects.EffectType<{
    sceneName: string,
    sceneId: string
}> = {
    definition: {
        id: `${PLUGIN_ID}:change-scene`,
        name: `${PLUGIN_NAME}: Change Scene`,
        description: "Changes the active scene in Meld Studio",
        icon: "fad fa-tv",
        categories: ["common"]
    },
    optionsTemplate: `
    ...
    onTriggerEvent: async (event) => {
        if (event.effect.sceneId) {
            MeldRemote.showSceneById(event.effect.sceneId);
        } else {
            MeldRemote.showSceneByName(event.effect.sceneName);
        }
        return true;
    }
}